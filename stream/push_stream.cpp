...
*/
    FILE *stream::open_flv(char *flv_name) {
        FILE *fp = nullptr;
    ...
    //connect rtmp server
    RTMP *stream::conect_rtmp_server(char *rtmpaddr) {
        RTMP *rtmp = nullptr;
        //1. 创建RTMP对象,并进行初始化
        rtmp = RTMP_Alloc();
        if (!rtmp) {
    ...
    //向流媒体服务器推流
    void stream::send_data(FILE *fp, RTMP *rtmp) {
    ...
    //分配RTMPPacket空间
    RTMPPacket *stream::alloc_packet() {
    ...
    */
    int stream::read_data(FILE *fp, RTMPPacket **packet) {
    ...
    void stream::publish_stream(char *flv, char *rtmpaddr) {
